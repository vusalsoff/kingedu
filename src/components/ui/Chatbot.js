...
const MENECER_TOVSIYESI = `
  <div class="manager-recommendation-box" style="margin-top: 15px; padding-top: 10px; border-top: 1px solid rgba(255,107,0,0.2);">
      <p class="info-card-text" style="font-size: 13px; margin-bottom: 8px;"><b class="highlight-orange" style="color:var(--primary);">🎯 Şahzadənin Ətraflı Məsləhəti:</b> Təlim proqramlarımızın qeydiyyat şərtləri, endirimli investisiya paketləri, ödəniş imkanları və dərslərə yerinizi dərhal bron etmək üçün peşəkar menecerimizlə birbaşa əlaqə saxlaya bilərsiniz:</p>
      <a href="https://wa.me/994103790874" target="_blank" class="manager-btn" style="display:inline-block; padding:8px 15px; background:#25D366; color:white; font-weight:bold; border-radius:20px; text-decoration:none; font-size: 13px;">💬 Menecerə WhatsApp-da Ətraflı Yaz</a>
  </div>
`;
    ...
    try {
      if (
        ...
        setMessages(prev => [...prev, {
          ...
          text: `<div class="info-card-container">
              <div class="info-card-title" style="font-weight:bold; color:var(--primary); margin-bottom:8px; font-size:15px;">👑 Rəhbərlik və Korporativ Əlaqə</div>
      ...
      if (txt.includes("tarixi") || txt.includes("nevaxtqurulub")) {
        ...
        setMessages(prev => [...prev, {
          sender: "bot",
          text: `<div class="info-card-container">
              <div class="info-card-title" style="font-weight:bold; color:var(--primary); margin-bottom:8px; font-size:15px;">📅 KİNG Education Company MMC-nin Qısa Tarixi</div>
      ...
      if (txt.includes("diplom")) {
        ...
        setMessages(prev => [...prev, {
          sender: "bot",
          text: `<div class="info-card-container">
              <div class="info-card-title" style="font-weight:bold; color:var(--primary); margin-bottom:8px; font-size:15px;">🎓 Rəsmi Diplom Nümunəsi və Şərtləri</div>
              ...
              <a href="${DIPLOM_IMG_URL}" target="_blank" class="dynamic-link-btn" style="display:inline-block;margin-top:10px;padding:8px 12px;background:var(--primary);color:white;text-decoration:none;border-radius:20px;font-weight:bold; font-size: 13px;">🖼 Rəsmi Diplom Nümunəsinə Bax</a>
      ...
      if (txt.includes("sertifikat")) {
        ...
        setMessages(prev => [...prev, {
          sender: "bot",
          text: `<div class="info-card-container">
              <div class="info-card-title" style="font-weight:bold; color:var(--primary); margin-bottom:8px; font-size:15px;">📄 Rəsmi Sertifikat Nümunəsi və Keçərliliyi</div>
              ...
              <a href="${SERTIFIKAT_IMG_URL}" target="_blank" class="dynamic-link-btn" style="display:inline-block;margin-top:10px;padding:8px 12px;background:var(--primary);color:white;text-decoration:none;border-radius:20px;font-weight:bold; font-size: 13px;">🖼 Rəsmi Sertifikat Nümunəsinə Bax</a>
          </div>${MENECER_TOVSIYESI}`
        }]);
        setIsLoading(false);
        return;
      ...
      if (tapilanData) {
        ...
        if (tapilanData.tip === "tek") {
          resultHTML += `<div class="info-card-title" style="font-weight:bold; color:var(--primary); margin-bottom:8px; font-size:15px;">🎯 ${tapilanData.basliq} Təlim Proqramı</div>`;
          resultHTML += `<p class="info-card-text" style="margin-bottom:10px;">King Education Company MMC olaraq təqdim etdiyimiz bu peşəkar istiqamət gənclərin karyera inkişafına xüsusi töhfə verir.</p>`;
          tapilanData.siyahis.forEach((item) => {
            if (item.val) {
              resultHTML += `<p class="info-card-text" style="margin-bottom:6px;"><b>${item.label}:</b> <span style="font-weight: normal; opacity: 0.9;">${item.val}</span></p>`;
            }
          });
          resultHTML += `</div>${MENECER_TOVSIYESI}`;
        } else if (tapilanData.tip === "butun_kurslar") {
          resultHTML += `<div class="info-card-title" style="font-weight:bold; color:var(--primary); margin-bottom:8px; font-size:15px;">📚 King Education MMC - Bütün Aktiv Kurslarımız</div>`;
          resultHTML += `<p class="info-card-text" style="margin-bottom:12px;">Şirkətimiz tərəfindən təşkil edilən peşəkar kurslar siyahısı və onların əsas şərtləri aşağıdadır:</p>`;
          tapilanData.liste.forEach((k, idx) => {
            resultHTML += `<div style="background:rgba(255,107,0,0.05); padding:10px; border-radius:8px; margin-bottom:8px; border-left:4px solid var(--primary);">`;
            resultHTML += `<p class="info-card-text" style="margin-bottom:4px;"><b>${idx + 1}. ${k.basliq}</b></p>`;
            resultHTML += `<p class="info-card-text" style="font-size:13.5px; margin-bottom:2px;">⏳ <b>Müddət:</b> ${k.muddet} | 💻 <b>Format:</b> ${k.format || "Onlayn"} | 💳 <b>Qiymət:</b> <span class="highlight-orange" style="color:var(--primary); font-weight:bold;">${k.qiymet}</span></p>`;
            if (k.kimler)
              resultHTML += `<p class="info-card-text" style="font-size:13px; opacity:0.8; margin-top: 4px;">👥 <b>Kimlər üçündür:</b> ${k.kimler}</p>`;
            resultHTML += `</div>`;
          });
          resultHTML += `</div>${MENECER_TOVSIYESI}`;
        } else if (tapilanData.tip === "butun_marafonlar") {
          resultHTML += `<div class="info-card-title" style="font-weight:bold; color:var(--primary); margin-bottom:8px; font-size:15px;">🎯 King Education MMC - Bütün Aktiv Marafonlarımız</div>`;
          resultHTML += `<p class="info-card-text" style="margin-bottom:12px;">Bütün marafonlarımız xüsusi endirimlə <b class="highlight-orange" style="color:var(--primary);">cəmi 20 AZN</b> təşkil edir. İstədiyiniz marafona qoşulmaq üçün birbaşa aşağıdakı düyməni istifadə edə bilərsiniz:</p>`;
          tapilanData.liste.forEach((m, idx) => {
            ...
            resultHTML += `<div style="background:rgba(255,107,0,0.05); padding:10px; border-radius:8px; margin-bottom:8px; border-left:4px solid var(--primary);">`;
            resultHTML += `<p class="info-card-text" style="margin-bottom:4px;"><b>${idx + 1}. ${m.basliq}</b></p>`;
            resultHTML += `<p class="info-card-text" style="font-size:13.5px; margin-bottom:8px;">⏳ <b>Müddət:</b> ${m.muddet} | 💳 <b>İnvestisiya:</b> <span class="highlight-orange" style="color:var(--primary); font-weight:bold;">20 AZN</span></p>`;
            resultHTML += `<a href="${waLink}" target="_blank" class="manager-btn" style="font-size:12px; padding:6px 14px; background:#25D366; color:white; border-radius:15px; text-decoration:none; display:inline-block; font-weight:bold;">💬 Bu Marafon üçün Menecerə Yaz</a>`;
            resultHTML += `</div>`;
          });
          resultHTML += `</div>`;
        }
      ...
      {!isOpen && (
        <div className="chatbot-launcher" onClick={() => setIsOpen(true)}>
          <div className="launcher-avatar">
            <Image src="/img/Sahzade ai.jpeg" alt="Şahzadə AI" width={60} height={60} className="avatar-img" />
          </div>
        </div>
      )}
        ...
        <div className="chatbot-window">
          <div className="chatbot-header">
            <div className="header-info">
              <Image src="/img/Sahzade ai.jpeg" alt="Şahzadə AI" width={40} height={40} className="avatar-img-small" />
              <div>
                <h3>Şahzadə 👑</h3>
          ...
          <div className="quick-replies" style={{ padding: '0.5rem 1rem', background: 'var(--bg-alt)', borderTop: '1px solid var(--border-color)', margin: 0, overflowX: 'auto', flexWrap: 'nowrap', whiteSpace: 'nowrap' }}>
